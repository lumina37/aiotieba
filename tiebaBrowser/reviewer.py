...
class RegularExp(object):
    ...
    contact_exp = re.compile(
        r'(\+|加|联系|私|找).{0,2}我|(d|滴)我|(私|s)(信|我|聊)|滴滴|di?di?', re.I)
    contact_rare_exp = re.compile(r'(在|看|→|👉|☞放).{0,3}(我|俺|偶).{0,3}((贴|帖)(子|里)|关注|主页|主业|资料|签名|尾巴|简介|(头|投)(像|象))|(头|投).(像|象)|主.页|(关注|主页|主业|资料|签名|尾巴|简介|(头|投)(像|象))(有|安排|上车)|威(信|辛)|v信|(\+|加|联系|了解|十|＋|私|找|伽).{0,2}(微信|威|薇|徽|微|wx|v|q|企鹅|扣扣|❤|蔻|寇|偶)|(十|＋|伽).{0,2}(我|俺|偶)|(威|薇|威|微|wx|v|企鹅|扣扣|❤|蔻|寇).{0,2}(:|：|号|楼下|签名)|q.?\d{6,11}|有意.{0,3}(s|私)|连细方式|罔址|芷|个.?性.?签.?名|簽|萜|貼|∨|Ⓥ|✙|➕|徾', re.I)
    ...
    course_check_exp = re.compile(
        r'交流群|课程|徒弟|素材|资料|教(程|学)|学习|邮箱|留言|扣.?1|(想|要)学|提升|小伙伴')

    app_nocheck_exp = re.compile(
        r'tao寳|tb口令|(淘宝|抖音).{0,2}(号|hao)|绿色.{0,2}平台|赛事预测|【支付宝】|解封微信|扫码.{0,3}送红包|关注.{0,2}微博|语音社交|帮注册|茨|视频助手|呅|嬴|瀛|複制|收soul.?天|↓住|流亮|聊天.?有惊喜|租房神器|口令打开淘宝|交易猫|中国女足\.我爱你', re.I)
    app_exp = re.compile(
        r'拼(夕夕|多多|dd)|京(东|d)|抖音|支付宝|淘.?宝|火山小视频|微信|饿.?了.?么|美.?团|唯品会|苏宁|快手|易购|app.{0,4}下载|双(11|十一)|语音平台|起点读书|需要.{0,2}app', re.I)
    app_check_exp = re.compile(
        r'点一下|点赞|任务|复制|账号|绿色|开店|店铺|运营|搜|红.?包|福利|推广|聘|免费|(广告|精准)投放|活动|助力|特价|邀请码|大牌优惠券|佣.?金|变现')

    business_exp = re.compile(
        r'[^\d]1[345789]\d{9}[^\d]|(高仿|复刻|购).{0,3}(鞋|包|表)|鞋文化|潮.{0,2}鞋|呺|核雕|莆田|工厂直供|品质保证|无中介费|免费鉴定|价格美丽|潮牌复刻|商铺改造|奢潮|本店主营|(实惠|羊毛).*群|撸货|线报|厂家货源|助力微商|绿色正规行业|价格可谈|金钱自由|零售商|网赌|火爆招商|对接工厂|实拍区别|臻情|钜献|火热预约|电子商务|有限公司|公司.{0,2}注册|蓷|廣|教育品牌|引流招商|自主研发|全国客服电话|回馈客户|可接定制|培训辅导|(投放).{0,2}广|高(转化|收益)|借贷|朋友推荐.*产品|区块链|进裙|福利.*进群|华强北|AirPods|玻尿酸', re.I)

    job_nocheck_exp = re.compile(
        r'成立工作室|想做单|宝妈[^妈]|(跟着|动手指).{0,2}赚钱|免费入职|(招|收).{0,4}(临时工|徒|代理)|(在家|轻松).{0,2}(可|能|没事).?做|时间自由|煎直|兼耳只|勉费|上迁.*没问题|不收.?任何费用|包食宿|vx辅助|闲时无聊|坚持.*日入过|语音聊天|有声书|读物配音|一对一直播|线上兼职|有想.*的(朋友|兄弟)|包学会|包分配|工作(轻松|简单)|不收(米|钱)|名额有限|在家.{0,4}操作|转发朋友圈|手工活|号商|(刷|做)(单|销量)', re.I)
    job_exp = re.compile(
        r'佣金|押金|会费|培训|结算|(日|立)(结|洁)|高佣|餐补|想做的|有兴趣|(急|长期|大量)招|招(募|聘)|稳赚|(一|每)(天|日|月)\d{2,3}|(日|月)(入|进).{0,2}(元|块|百|佰|万|千|w)|(利润|收益|工资|薪资|收入|福利|待遇)(高|好|\d{3,}|稳定)|(高|好)(利润|收益|工资|薪资|收入|福利|待遇)|低(风险|投入)|风险低|正规合法|合作愉快|手机.*就(行|可)|(有|一).?手机|扶持|长期单', re.I)
    ...
    game_check_exp = re.compile(
        r'神豪|托|演员|充值|试玩|内玩|限时|速来|体验|开服|内测|资源|福利|每天都有|推广|关注下|内部号|扶持|嘴严|追杀|特权礼包')
    ...
    hospital_exp = re.compile(
        r'医院.*好不好|狐臭|痔疮|性腺|阳痿|早泄|不孕不育|前列腺|妇科|会所|手相|(邪|手)淫.{0,3}危害')

    lv1_exp = re.compile(
        r'公众号|传媒|新媒体|婚恋|财经|鱼胶|信︄用卡|出租|塔罗|代骂|消灾|问卷调查|有意者|急需.{0,10}钱|(免费|资源)分享|懂(的|得)来|代练|戒邪淫|我发表了一篇图片')
