...
class RegularExp(object):
    ...
    app_nocheck_exp = re.compile(
        'tao寳|tb口令|(淘宝|抖音).{0,2}(号|hao)|绿色.{0,2}平台|赛事预测|【支付宝】|解封微信|扫码.{0,3}送红包|关注.{0,2}微博|语音社交|帮注册|茨|视频助手|呅|嬴|瀛|複制|收soul.?天|↓住|流亮|聊天.?有惊喜|租房神器|面团|口令打开淘宝|交易猫', re.I)
    app_exp = re.compile(
    ...
    business_exp = re.compile(
        '[^\d]1[345789]\d{9}[^\d]|(高仿|复刻|购).{0,3}(鞋|包|表)|鞋文化|潮.{0,2}鞋|呺|核雕|莆田|工厂直供|品质保证|免费鉴定|价格美丽|潮牌复刻|奢潮|本店主营|(实惠|羊毛).*群|撸货|线报|厂家货源|助力微商|绿色正规行业|价格可谈|金钱自由|零售商|网赌|火爆招商|对接工厂|实拍区别|臻情|钜献|火热预约|电子商务|有限公司|公司.{0,2}注册|蓷|廣|教育品牌|引流招商|自主研发|全国客服电话|回馈客户|可接定制|培训辅导|(投放).{0,2}广|高(转化|收益)|借贷|朋友推荐.*产品|区块链|进裙|福利.*进群|华强北|AirPods|玻尿酸', re.I)
    ...
    game_nocheck_exp = re.compile(
        '(招|找).{0,4}(托|拖|内部|人员|体验员)|(狗|手游|游戏|内部).?(托|拖|脱|资格|号)|(托|拖|脱)(裙|群)|进游(有|给)|上线就送|来人体验|刺激玩家消费|(走心|手游|本吧|本帖|本贴).{0,2}推荐|体验氪金大佬|等级直接调|跟得上强度|(真实|有效)(冲|充)值|(等级|线下)扶持|茗额|游推荐|特权礼包|新区.{0,2}开|霸服|喜欢玩.*仙侠|(日|天|送|领|给你|免费).{0,2}(648|元宝)|手游.{0,2}(招|爱好者)|大.?不.?同.{1,4}网|官网.{1,7}变态|手游官网|B（版）T（本）|dbt.?apk|B-T版本|в|(当|做)游戏主播|来就.{0,2}送|蝣|遊|戲|號|侗|皮肤最新获取|鎹|區|輑|避免玩家流失')
