...
class RegularExp(object):
    ...
    contact_exp = re.compile(
        '(\+|加|联系|私|找).{0,2}我|(d|滴)我|(私|s)(信|我|聊)|滴滴|di?di?|迬', re.I)
    ...
    app_nocheck_exp = re.compile(
        'tao寳|tb口令|(淘宝|抖音).{0,2}(号|hao)|绿色.{0,2}平台|赛事预测|【支付宝】|解封微信|扫码.{0,3}送红包|关注.{0,2}微博|帮注册|茨|视频助手|呅|開|嬴|瀛|收soul.?天|↓住|流亮|聊天.?有惊喜|【娛】|缅甸.{0,3}娱乐|租房神器|浮力车车|面团|口令打开淘宝', re.I)
    app_exp = re.compile(
    ...
    business_exp = re.compile(
        '[^\d]1[345789]\d{9}[^\d]|(高仿|复刻|购).{0,3}(鞋|包|表)|鞋文化|潮.{0,2}鞋|呺|核雕|莆田|工厂直供|品质保证|价格美丽|潮牌复刻|(实惠|羊毛).*群|撸货|线报|厂家货源|助力微商|#后期#.*做熟悉|绿色正规行业|价格可谈|金钱自由|零售商|网赌|火爆招商|电子商务|有限公司|公司.{0,2}注册|蓷|廣|教育品牌|引流招商|自主研发|回馈客户|可接定制|培训辅导|(投放).{0,2}广|高(转化|收益)|借贷|朋友推荐.*产品|区块链|进裙|福利.*进群|华强北|AirPods|玻尿酸', re.I)
    ...
    game_nocheck_exp = re.compile(
        '(招|找).{0,4}(托|内部|人员)|(狗|手游|游戏|内部).?(托|拖|脱)|(托|拖|脱)(裙|群)|进游(有|给)|上线就送|喜欢玩.*仙侠|(日|天|送|领|给你|免费).{0,2}648|私服|手游.{0,2}招|大不同.{1,3}网|dbtapk|B-T版本|提灯与地下城|(当|做)游戏主播|来就.{0,2}送|蝣|遊|戲|號|侗|皮肤最新获取|鎹|區|輑|六七手游')
    ...
    maipian_exp = re.compile(
        '(下|↓).{0,3}有惊喜|浮力车|网红.{0,3}作品|成人看的|小情侣|注意身体|推荐.{0,3}资源|回复.*:你(帖|贴).*可以看|自己上微.?薄|自己.*捜|都有.*看我关注的人|范 做|看偏神器|学姐给我吃|推荐发展对象|^麦片$|卖淫|嫂子直接.*那个|小哥哥们.*看我|进去.*弄得喷水|(看|有)女神|噜.?个月|肾不好|箹|〖.〗|诚人|28.ME|湖北厕所|\.xyz')
