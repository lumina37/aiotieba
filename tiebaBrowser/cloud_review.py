...
class RegularExp(object):
    ...
    contact_rare_exp = re.compile(
        '(在|看|→|👉|☞放).{0,3}(我|俺|偶).{0,3}((贴|帖)(子|里)|关注|主页|主业|资料|签名|尾巴|简介|(头|投)(像|象))|(头|投).(像|象)|主.页|(关注|主页|主业|资料|签名|尾巴|简介|(头|投)(像|象))(有|安排|上车)|威(信|辛)|v信|(\+|加|联系|了解|十|＋|私|找|伽).{0,2}(微信|威|薇|徽|微|wx|v|q|企鹅|扣扣|❤|蔻|寇|偶)|(十|＋|伽).{0,2}(我|俺|偶)|(威|薇|威|微|wx|v|企鹅|扣扣|❤|蔻|寇).{0,2}(:|：|号|楼下|签名)|q.?\d{6,11}|有意.{0,3}(s|私)|连细方式|罔址|芷|个.?性.?签.?名|簽|萜|貼|∨|Ⓥ|✙|➕|徾', re.I)
    ...
    app_nocheck_exp = re.compile(
        'tao寳|tb口令|(淘宝|抖音).{0,2}(号|hao)|绿色.{0,2}平台|赛事预测|【支付宝】|解封微信|扫码.{0,3}送红包|关注.{0,2}微博|帮注册|茨|视频助手|呅|開|嬴|最大1111|收soul.?天|↓住|5.{2}·|流亮|你的够用吗？|聊天.?有惊喜|y͏u͏n͏g͏e͏8͏8͏7͏|a͏i͏s͏h͏e͏n͏5͏7͏6͏|【娛】|缅甸.{0,3}娱乐|租房神器|浮力车车|面团|口令打开淘宝', re.I)
    app_exp = re.compile(
        '拼(夕夕|多多|dd)|京(东|d)|抖音|支付宝|淘.?宝|火山小视频|微信|饿.?了.?么|美.?团|唯品会|苏宁|易购|app.{0,4}下载|双(11|十一)|语音平台|起点读书|需要.{0,2}app', re.I)
    app_check_exp = re.compile(
        '点一下|点赞|任务|复制|账号|绿色|开店|店铺|运营|搜|红.?包|福利|推广|聘|免费|(广告|精准)投放|活动|助力|特价|邀请码|佣.?金')

    business_exp = re.compile(
        '[^\d]1[345789]\d{9}[^\d]|(高仿|复刻|购).{0,3}(鞋|包|表)|鞋文化|潮.{0,2}鞋|呺|核雕|莆田|工厂直供|品质保证|价格美丽|潮牌复刻|(实惠|羊毛).*群|撸货|线报|厂家货源|助力微商|迬|#后期#.*做熟悉|绿色正规行业|价格可谈|金钱自由|零售商|网赌|火爆招商|电子商务|有限公司|公司.{0,2}注册|蓷|廣|瀛|教育品牌|引流招商|自主研发|回馈客户|可接定制|培训辅导|(投放).{0,2}广|高(转化|收益)|借贷|朋友推荐.*产品|区块链|进裙|福利.*进群|华强北|AirPods|玻尿酸', re.I)
    ...
    job_exp = re.compile(
        '佣金|押金|会费|培训|结算|(日|立)(结|洁)|高佣|餐补|想做的|有兴趣|急招|招(募|聘)|稳赚|(一|每)(天|日|月)\d{2,3}|(日|月)(入|进).{0,2}(元|块|百|佰|万|w)|(利润|收益|工资|薪资|收入|福利|待遇)(高|好|\d{3,})|(高|好)(利润|收益|工资|薪资|收入|福利|待遇)|低(风险|投入)|风险低|正规合法|合作愉快|手机.*就(行|可)|(有|一).?手机|扶持|长期单', re.I)
    job_check_exp = re.compile(
        '(暑假|临时|短期)工|合伙|兼职|主播|声播|签约|艺人|模特|陪玩|写手|点赞员|接单|工作室|工厂|手工|项目|电商|游泳健身|有声(读物|书)|创业|自媒体|加盟|副业|代理|(免费|需要|诚信|诚心)(带|做)|想(赚|挣)钱|不甘.?现状|微商|微信|投资|写好评|不嫌少|需工作|形象好|气质佳|赢.{0,2}奖金')

    game_nocheck_exp = re.compile(
        '(招|找).{0,4}(托|内部|人员)|(狗|手游|游戏|内部).?(托|拖|脱)|(托|拖|脱)(裙|群)|2337646|进游(有|给)|上线就送|喜欢玩.*仙侠|(日|天|送|领|给你|免费).{0,2}648|私服|手游.{0,2}招|大不同.{1,3}网|B-T版本|提灯与地下城|(当|做)游戏主播|来就.{0,2}送|蝣|遊|戲|號|侗|hop132|皮肤最新获取|鎹|區|輑|六七手游')
    game_exp = re.compile('手游|游戏|玩家|仙侠|国战')
    game_check_exp = re.compile(
        '神豪|托|演员|充值|试玩|新区|内玩|体验|开服|霸服|资源|福利|每天都有|推广|关注下|内部号')
    ...
    maipian_exp = re.compile(
        '(下|↓).{0,3}有惊喜|浮力车|网红.{0,3}作品|成人看的|小情侣|注意身体|推荐.{0,3}资源|回复.*:你(帖|贴).*可以看|自己上微.?薄|自己.*捜|都有.*看我关注的人|experience\+.{0,8}#.*#|范 做|看偏神器|学姐给我吃|推荐发展对象|脱单|^麦片$|卖淫|嫂子直接.*那个|小哥哥们.*看我|进去.*弄得喷水|(看|有)女神|噜.?个月|鲁管|肾不好|[\u0101-\u017e]|箹|\{.?\}|〖.〗|诚人|28.ME|湖北厕所|\.xyz')
    ...
    lv1_exp = re.compile(
        '公众号|传媒|新媒体|婚恋|财经|鱼胶|信︄用卡|看手相|出租|塔罗|代骂|消灾|问卷调查|有意者|(邪|手)淫.{0,3}危害|急需.{0,10}钱|(免费|资源)分享|懂(的|得)来|代练')
