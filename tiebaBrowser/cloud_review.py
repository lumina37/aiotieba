...
class RegularExp(object):
    ...
    game_nocheck_exp = re.compile(
        '(招|找).{0,4}(托|拖|内部|人员|体验员)|(狗|手游|游戏|内部).?(托|拖|脱|资格|号)|(托|拖|脱)(裙|群)|进游(有|给)|上线就送|来人体验|刺激玩家消费|(走心|手游|本吧|本帖|本贴).{0,2}推荐|体验氪金大佬|等级直接调|真实充值|游推荐|特权礼包|新区.{0,2}开|霸服|喜欢玩.*仙侠|(日|天|送|领|给你|免费).{0,2}(648|元宝)|手游.{0,2}(招|爱好者)|大.?不.?同.{1,4}网|官网.{1,7}变态|手游官网|B（版）T（本）|dbt.?apk|B-T版本|в|(当|做)游戏主播|来就.{0,2}送|蝣|遊|戲|號|侗|皮肤最新获取|鎹|區|輑|六七手游|2046')
    game_exp = re.compile('手游|游戏|玩家|仙侠|国战|新区')
    game_check_exp = re.compile(
        '神豪|托|演员|充值|试玩|内玩|限时|速来|体验|开服|资源|福利|每天都有|推广|关注下|内部号')
