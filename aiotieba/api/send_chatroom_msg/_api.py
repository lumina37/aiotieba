...
async def construct_request_data(blcpcore, room_id, uk, user_id, origin_id, name, portrait, text, forum_id, level, vip,
                                 glevel, atdata=None, robot=-1):
    constants = AppConstants()

    # 构造app_safe_ext
    app_safe_ext = {'haotianjing': {'zid': blcpcore.account.z_id if blcpcore.account.z_id else ""}}
    ...
    if vip:
        # 开通了贴吧大会员，更新名称颜色
        namedata['text'].update({'text_color': {
            "day": "CAM_X0301",
            "night": "CAM_X0301",
            "type": 2
        }})
    main_data.append(namedata)
    main_data.append({
        'icon': {
            'height': 15,
            'priority': 5,
    ...
    main_data.append({
        'icon': {
            'height': 12,
            'priority': 2,
            'schema': 'https://tieba.baidu.com/mo/q/wise-bawu-core/forum-level?customfullscreen=1&forum_id=' + str(
                forum_id) + '&nonavigationbar=1&obj_locate=5&portrait=' + portrait + '?t=' + str(int(time.time())),
            'type': '4',
            'url': 'local://icon/icon_level_' + '{:02d}'.format(level) + '?type=webp',
            'width': 16
        },
        'type': 2
    })
    ...
    request_data = {
        ...
        'event_list': [{'event': 'CClickSendBegin', 'timestamp_ms': 0},
...
async def send_request(blcpcore, request_data):
    ...
    loginBLCPRequest.RpcBody = blcpcore.buildRpcBody(serviceId=3, methodId=185,
...
async def request(blcpcore: BLCPCore, room_id: int, uk: int, user_id: int, origin_id: int, name: str, portrait: str,
                  text: str,
                  forum_id: int, level: int, vip: bool, glevel: int, atdata: list[dict] = None, robot=-1):
    request_data = await construct_request_data(blcpcore, room_id, uk, user_id, origin_id, name, portrait, text,
                                                forum_id, level, vip, glevel, atdata, robot)
