...
在启用了指令管理器的贴吧的**任何位置**（可以是主题帖、回复或楼中楼）发送以下文字就能使用指令
...
中，`block`表示指令类型，`李彦宏`为参数1（待封禁的用户id），封禁测试为参数2（封禁理由）

需要注意的是@信息要变蓝才能确保v_guard收到指令请求。@v_guard的正确姿势请参考下方案例
...
![1_1](https://user-images.githubusercontent.com/48282276/171667469-6a70703e-08ca-4df7-aa7a-de5cb8b11e40.jpg)

+ 先`@v_guard`并确保变蓝

![1_2](https://user-images.githubusercontent.com/48282276/171667750-0cde4816-c755-4565-9d22-e1688a563dd7.jpg)

+ 再敲入<指令类型>`drop`和[参数1]封禁理由“test1234”，并发布回复

![1_3_1](https://user-images.githubusercontent.com/48282276/171667893-68643002-8abb-47a1-87d6-72090c8f5a41.jpg)

![1_3_2](https://user-images.githubusercontent.com/48282276/171670836-2a575815-be7f-45fe-8f3c-a5b32e57e10a.jpg)

+ 指令生效，主题帖被删除，楼主`v_guard`被封十天

![1_4_1](https://user-images.githubusercontent.com/48282276/171667952-fec55cfc-1a65-426a-8b2f-7941f626a441.png)

![1_4_2](https://user-images.githubusercontent.com/48282276/171668105-2d4115c8-e63a-447d-a827-42159477658e.png)

![1_4_3](https://user-images.githubusercontent.com/48282276/171668249-798c08f6-2fba-4fea-8186-079937a291e8.png)

### 如何在app不依赖任何现有发言封禁任意用户十天

+ 复制用户的`tieba_uid`

![2_1](https://user-images.githubusercontent.com/48282276/160992857-c4f29b1c-1b46-4074-b1c5-b6f83a62de46.jpg)

+ 在这个吧的任何位置输入`@v_guard block`并确保@变蓝，然后敲一个空格，最后把刚刚复制的那坨东西粘贴进去，点击回复

![2_2](https://user-images.githubusercontent.com/48282276/160992862-341c666c-5a45-40f2-ab75-ab098b456235.jpg)

+ 指令生效，`tieba_uid=316431307`的用户`kk不好玩`被封十天

![2_3](https://user-images.githubusercontent.com/48282276/160992854-04d2128b-934c-4ba1-9a22-11d768c6989d.png)

### 如何删除一条违规回复并顺便封层主三天

+ 点一下需要删除的楼层拉起回复框，输入回复内容`@v_guard drop3`并确保@变蓝，点击回复按钮

![3_1](https://user-images.githubusercontent.com/48282276/160995393-903511ae-9ec3-4055-890e-e144a7540fac.jpg)

+ 指令生效，楼层被删，层主`v_guard`被封三天

![3_2_1](https://user-images.githubusercontent.com/48282276/160995720-e8a288ae-e1d7-477a-a5d0-3a5622d30b1d.png)

![3_2_2](https://user-images.githubusercontent.com/48282276/160995724-63c9afd8-bf41-454b-8647-7858461d6c72.png)

### 网页端如何解封任意用户

+ 点开用户主页，在链接里找到一串`tb.1.`开头的东西，这个东西叫`portrait`，每个用户都有唯一的`portrait`，把它复制出来

![4_1](https://user-images.githubusercontent.com/48282276/160996223-8afa7be5-1051-4cdd-9d4c-0e05044774e9.png)

+ 在这个吧的任何位置输入`@v_guard unblock`，然后敲一个空格，最后把刚刚复制的那坨东西粘贴进去，点击回复

![4_2](https://user-images.githubusercontent.com/48282276/160996234-67fb1940-9276-4069-8eb7-c67f532620d5.png)

+ 指令生效，`portrait=tb.1.8277e641.gUE2cTq4A4z5fi2EHn5k3Q`的用户`kk不好玩`被解封

![4_3](https://user-images.githubusercontent.com/48282276/160996957-66f0976a-976a-42f4-8166-2d461561c45a.png)

## 权限级别说明

权限级别可以通过指令`set` `white` `black` `reset`修改
...
+ `1` 非吧务的优秀创作者 - 可以使用`recom_status` `vote_stat` `recommend` `ping`指令<br>
...
> 但你需要避免文字被解析为链接，一般来说删去前置的https是常用做法
...
> 这里为了避免文字被解析为链接，我删掉了链接开头的`https://`
> 
...
> `reason` 可选参数，封禁理由
> 
> `id` `用户名`或`portrait`或包含`tieba_uid`的字符串或包含`user_id`的字符串，各自对应一种满足不重复性质的用户标识符<br>
> + `用户名` 请注意与`昵称`相区别，每个用户名只会指向唯一的用户
> + `portrait` 每个用户都有唯一的`portrait`，一般是以`tb.1.`开头的一串字符串<br>
> 用网页端处理用户时，推荐使用这个`portrait`方法予以精准定位
> + 包含`tieba_uid`的字符串 `tieba_uid`需要被两个`#`号包围，如`#12345678#`<br>
> 在较新版贴吧app的用户主页的用户头像下方有个`ID:12345678`的东西，点右侧的复制按钮复制出来一串东西，就能作为这里的参数<br>
> 用app处理无用户名的用户，或者用户名很复杂的用户时，推荐使用这个包含`tieba_uid`的字符串方法予以精准定位
> + 包含`user_id`的字符串 `user_id`需要被两个`/`号包围，如`/12345678/`<br>
...
> 在使用该指令时请特别注意指令的发送位置，以免产生意料之外的效果<br>
...
> ***开发者说***
> 
> `block` `delete` `black`的组合，用于提升删封循操作效率
...
> `permission` 必填参数，新的图片黑名单级别，`-5`意味着当云审查发现该图片时会删封10天，`-4`删封3天，`-3`删封1天，`-2`仅删帖，`-1`仅屏蔽<br>
...
> ***功能***
> 
> 统计主题帖每一楼层的包含`keyword`且用户等级大于等于`min_level`的楼中楼数量，并按用户的`user_id`去重

> ***参数说明***
> 
> `limit` 按票数顺序返回得票最高的`limit`条结果<br>
> `keyword` `min_level` 文字内容包含`keyword`且用户等级大于等于`min_level`的楼中楼回复会视作一票
